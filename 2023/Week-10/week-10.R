...
df <- numbats |>
  select(recordID, year, month, wday, hour, day,
...
df_map <- df|>
  mutate(thresh=case_when(year<2020 ~ "Before 2020",
                          year>=2020 & year!=2023 ~ "2020 to 2022",
                          year == 2023 ~ "As of 2023"))|>
  select(decimalLatitude, decimalLongitude,thresh )
